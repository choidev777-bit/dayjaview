...
- 검토한 디자인 원본 커밋: `da00c8f` (2026-08-15 `main`)
- 디자인 적용 기준: [ui_prototype_adaptation_plan.md](./ui_prototype_adaptation_plan.md) — 시각·화면 구성은 시안이 최상위이며 이 문서가 시안과 다르면 이 문서를 고친다.

> 2026-08-15 정정: 이전 기준으로 적혀 있던 커밋 `65324878db4ef92bb29c7fce21e63a1031c3be17`은 시안 저장소에 존재하지 않으며, 그 커밋을 근거로 쓴 구조·색상 서술은 전부 오류였다. 아래 §5.2와 §6의 관련 항목을 함께 정정했다.
...
디자이너 시안을 제품 화면으로 옮기는 기준 문서다. 버전 `1.0`으로 실제 시안(`da00c8f`)을 감사해 재작성했다.

필수 내용:

- 디자인 원본 저장소와 기준 커밋, 기준 커밋 추종 정책
- 시각 계층은 시안이, 데이터 계층은 기준 문서가 최상위라는 우선순위 규칙
- 그대로 유지할 자산: 토큰 39개, 로고, 순위 휠, 카드 구조, 트리맵 배치, 모션
- 교체할 구조: 화면 전환 상태 → 실제 route, 하드코딩 데이터 → 서버 계약, 데모 로그인 폼 → Google OAuth
- 시안 화면과 제품 화면의 매핑
- `홈 / 실시간 / 즐겨찾기 / 리서치` 앱 셸
- 화면·기능·공통 컴포넌트 구조
- 모바일 단일 열 420px 고정 규칙 (시안에 데스크톱 레이아웃이 없다)
- 393×852 목업 프레임의 데모 모드 격리
- 접근성 기준
...
디자이너 시안의 시각 언어와 화면 구성을 그대로 가져오고 데이터 계층만 제품용으로 교체한다.
...
- 작업 브랜치와 코드 소유권 설정
- 취약 의존성 업데이트
- 시안 화면 전환 상태를 실제 라우터로 교체
- 시안 토큰 39개 이식 (실측값 기준 — adaptation plan §7)
...
- 393×852 목업 프레임은 별도 demo wrapper로 이동
...
- [ ] 화면·데이터·스타일이 한 파일에 집중되지 않는다.
- [ ] URL로 화면과 식별자를 복원할 수 있다.
- [ ] 디자인 토큰이 한곳에서 관리된다.
- [ ] 시안의 브랜드·시각 언어가 그대로 재현된다.
- [ ] 홈 순위 휠이 시안대로 동작하며 스크린리더 중복과 키보드 이동이 보완됐다.
- [ ] 하단 앱 셸이 시안의 `홈 / 실시간 / 즐겨찾기 / 리서치` 4탭과 일치한다.
...
- [ ] 제품이 420px 단일 열로 렌더링되고 393×852 목업 프레임이 production route에 없다.
