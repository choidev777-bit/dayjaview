...
| `S3-API` | 3 | 인증된 핵심 REST와 realtime-ticket WebSocket을 기계 계약 위에 구현한다. 비로그인 데이터 0건, 1회용 ticket, sequence/full snapshot, saved/operator 분리 contract test가 통과하면 종료한다. | `INT-S2` | `apps/api/**`, `tests/api/**`, `tests/realtime-api/**` | Stage 3 다른 작업과 가능 | `gpt-5.6-sol / max` | `V-PY`, `V-CONTRACT`, API/WSS smoke, `V-DIFF` | `codex/s3-api-realtime` | 생성 준비; pushed `INT-S2` evidence commit을 exact base로 사용하고 S1 identity, S2 Event writer/outbox/read snapshot과 기계 계약을 직접 소비 |
| `S3-WEB-LIVE` | 3 | fixture adapter와 동일 컴포넌트에 실제 REST/WSS client, cache purge, sequence/reconnect, saved flow를 연결한다. mock/staging contract parity와 `V-WEB`가 통과하면 종료한다. | `INT-S2` | `apps/web/**` | Stage 3 다른 작업과 가능 | `gpt-5.6-sol / xhigh` | `V-WEB`, `V-CONTRACT`, client integration, `V-DIFF` | `codex/s3-web-live-adapter` | 생성 준비; pushed `INT-S2` evidence commit을 exact base로 사용하고 S1 fixture shell과 S2 versioned full snapshot·sequence 의미를 유지. staging/live 호출은 별도 검증 |
| `S3-REPLAY-ADAPTER` | 3 | 저장 capture를 제품 canonical market event 입력으로 바꾸는 bounded adapter와 fixture test를 구현한다. ordering/hash/clock/failure test만 통과시키고 capture 또는 replay 실행은 하지 않으면 종료한다. | `INT-S2` | `packages/adapters/market-replay/**`, `tests/market-replay-adapter/**` | Stage 3 다른 작업과 가능 | `gpt-5.6-sol / xhigh` | `V-PY`, synthetic fixture only, `V-DIFF` | `codex/s3-replay-adapter` | 생성 준비; pushed `INT-S2` evidence commit을 exact base로 사용하고 S2 canonical market event/order fence 의미에 맞춘 synthetic fixture만 사용. `B-MARKET-FIXTURE` 유지, 실제 capture·replay 금지 |
| `S3-INFRA-LOCAL` | 3 | API·worker·PostgreSQL·Redis의 local/CI Compose와 ARM64-compatible image 골격을 만든다. config 검증, local fixture health, secret 미포함 검사가 통과하면 종료한다. | `INT-S2` | `infra/deployment/**`, `infra/images/**`, `infra/operations/local*`, `tests/infra/**` | Stage 3 다른 작업과 가능 | `gpt-5.6-sol / xhigh` | `V-STACK`, image/config scan, `V-DIFF` | `codex/s3-local-stack` | 생성 준비; pushed `INT-S2` evidence commit의 identity/Infostock/reference/Event migrations와 API·worker 경계를 local fixture stack에 연결. `B-DEPLOY` 유지, 실제 배포·cloud mutation 금지 |
...
| `INT-S3` | API·실제 web adapter·replay adapter·local stack 통합 | `gpt-5.6-sol / max` | `codex/int-s3-first-vertical` | fixture end-to-end·auth·WSS·health smoke; replay 실행 없이 Stage 4 생성 | 생성 준비; pushed `INT-S2` exact base와 Stage 3 네 task의 실제 thread ID를 입력으로 사용 |
