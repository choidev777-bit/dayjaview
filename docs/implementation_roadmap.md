...
| `S1-INFOSTOCK` | 1 | 기존 수집된 280/280 테마 원본 전체를 PostgreSQL에 보존·정규화한다. 테마 master·설명·원본 식별자·source metadata, 날짜·본문을 포함한 history 39,696건, related stocks 6,629건과 당시/현재 관계, leader 코드·이름·표시값·원본 누락 상태, 현재 membership과 가능한 historical membership을 적재한다. raw payload와 normalized projection에 `collected_at`·`as_of`·`revision`·`lineage`를 남기고 idempotent 재적재·중복·결측·참조 무결성 검사와 품질 보고가 통과해야 한다. history duplicate 4건과 leader code missing 90건은 삭제·숨김·추정 보정하지 않고 명시적 quality status로 보존하며, S6 Daily worker가 같은 schema·revision 모델로 증분 갱신할 계약을 고정하면 종료한다. | `INT-S0` | `packages/infostock/**`, `apps/worker-batch/infostock/**`, `infra/migrations/*infostock*`, `tests/infostock/**` | Stage 1 다른 작업과 가능 | `gpt-5.6-sol / max` | `V-PY`, PostgreSQL 16 full-import·idempotency·quality integration, `V-DIFF` | `codex/s1-infostock-store` | 진행 중; thread `019ffe83-b42d-7963-a8ed-dd0fef4b6bb2`에 전체 데이터 범위 보정 전달. S1은 기존 수집본 DB화만 수행하고 신규 login/live browser 수집은 S6로 유지한다. 기존 수집본 부재·기준 수량 불일치는 합성 데이터로 대체하지 않고 blocker로 보고하며 `B-INFOSTOCK-AUTH`·`B-DATA-RIGHTS`는 유지 |
