...
예약 작업이 띄운 세션은 매 회 아래를 그대로 따른다. 사람이 직접 지시할 때는 이 절을 무시한다.

1. `git pull --ff-only` 후 **진행 현황판**에서 상태가 `대기`인 **첫 작업 하나**를 고른다. 없으면 아무것도 하지 않고 즉시 종료한다.
2. 그 줄을 `진행중`으로 바꾸고 **그 변경만** 커밋·push한다. (다른 세션이 같은 작업을 집는 것을 막는 잠금이다)
3. **그 작업 하나만** 수행한다. 다음 작업으로 넘어가지 않는다.
4. 검증은 CLAUDE.md 기준으로 **각 1회만**. 파이썬을 고쳤으면 `uv run pytest -q` 1회, `apps/web`을 고쳤으면 lint·typecheck·test 각 1회. 통과하면 더 돌리지 않는다.
5. 커밋하고 **push**한다. 현황판을 `완료`로 바꾸고 근거 한 줄(무엇을 했는지)을 적어 함께 push한다.
6. 막히면(외부 관문·승인 필요·스스로 해결 불가) `막힘`으로 바꾸고 이유 한 줄을 적어 push한 뒤 **종료**한다. 다음 세션이 그다음 작업을 집는다.
7. `진행중`인데 그 줄의 마지막 커밋이 **60분 이상 전**이면 이전 세션이 죽은 것이다. 워킹트리를 정리(`git checkout -- .` 또는 이어서 완료)하고 그 작업을 다시 집는다.

**금지 (이전 에이전트가 시간을 낭비한 방식)**
- 한 세션에서 여러 작업을 처리하지 않는다.
- 새 브랜치를 만들지 않는다. `main` 하나만 쓴다.
- 요청받지 않은 리팩터링·문서 생성·추가 테스트를 하지 않는다.
- 소스 파일 본문을 grep하는 테스트, manifest 검사 테스트, `*_contract.py` 신규 생성 금지.
- 같은 검증 명령을 두 번 이상 돌리지 않는다.
- 작업 범위를 스스로 줄이고 완료라고 하지 않는다. 못 하면 `막힘`으로 정직하게 적는다.

## 진행 현황판

이 표가 유일한 진행 상태 기록이다. 세션은 여기만 보고 다음 할 일을 정한다.

| 순서 | 작업 | 권장 모델·추론 | 상태 | 근거 |
|---:|---|---|---|---|
| 1 | B-8 codex분 정리·뉴스 수집 | Fable 5 / high | 대기 | |
| 2 | A-1 인포스탁 280테마 연결 | Opus 5 / high | 대기 | |
| 3 | A-4 상시 파이프라인·DB 영속화 | Fable 5 / max | 대기 | |
| 4 | A-5 테마 상세 데이터 | Opus 5 / high | 대기 | |
| 5 | A-7 관심 목록 실데이터 | Opus 5 / high | 대기 | |
| 6 | B-9 뉴스↔테마 매칭 | Opus 5 / high | 대기 | |
| 7 | B-10 근거 기반 LLM 요약 | Fable 5 / high | 대기 | |
| 8 | B-11 근거 UI | Opus 5 / high | 대기 | |
| 9 | C-12 인사이트 트리맵 | Opus 5 / high | 대기 | |
| 10 | C-0 디자이너 시안 정합 | Opus 5 / high | 대기 | |
| 11 | A-6 랭킹 부가 지표 | Opus 5 / high | 대기 | |
| 12 | D-13 장후 정합 | Fable 5 / max | 대기 | |
| 13 | D-15 운영자 콘솔 | Opus 5 / high | 대기 | |
| 14 | E-16 과거 주가 corpus | Fable 5 / high | 대기 | |
| 15 | E-17 온톨로지 | Fable 5 / max | 대기 | |
| 16 | E-18 TOP3 | Opus 5 / high | 대기 | |
| 17 | F-23 보안 점검 | Fable 5 / max | 대기 | |
| 18 | F-24 품질 점검 | Opus 5 / high | 대기 | |
| — | A-2 기준정보 실데이터 | Opus 5 / high | 막힘 | KRX·OpenDART API 키를 사용자가 발급해야 시작 가능 |
| — | A-3 키움 live 어댑터 | Fable 5 / max | 막힘 | 장 개장 시간 + 사용자 승인 필요 |
| — | D-14 인포스탁 증분 수집 | Fable 5 / high | 막힘 | live 호출 승인 필요 |
| — | E-19 유사사례 엔진 | Fable 5 / max | 막힘 | E-16·E-17 완료 후 해제. 2인 블라인드 평가 필요 |
| — | E-20 유사사례 화면 | Opus 5 / high | 막힘 | E-19 평가 통과 후 해제 |
| — | F-21 실 구글 로그인 | Opus 5 / high | 막힘 | 배포 origin·구글 콘솔 등록 필요 |
| — | F-22 운영자 계정 | Opus 5 / high | 막힘 | 배포 환경 필요 |
| — | F-25 실제 배포 | Fable 5 / max | 막힘 | 사용자 승인 필요 |

상태값: `대기` → `진행중` → `완료` / `막힘`. 사용자가 관문을 풀어주면 `막힘`을 `대기`로 바꾼다.
