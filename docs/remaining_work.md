...
- 작성일: 2026-08-15 (마지막 갱신 2026-08-17, 기준 commit `f90e9c6`)
...
## 0. 현재 상태 (2026-08-15 작성, `4672cd7` 기준)

> 이 절의 서술은 2026-08-15에 쓴 것이고, 그 뒤 `f90e9c6`까지 89커밋이 더 있다. **완료 상태는 [work_checklist.md](./work_checklist.md)가 상태판이다.** 이 절은 구조·실행 방법·함정만 보고, 무엇이 끝났는지는 상태판에서 확인한다.
...
숫자는 2026-08-15 측정값이다(파이썬 제외). **최신 파이썬 수치는 [work_checklist.md](./work_checklist.md) 머리말 한 곳에만 둔다.**

| 검증 | 명령 | 현재 기준 |
|---|---|---|
| 파이썬 | `uv run pytest -q` | 최신 수치는 work_checklist.md 머리말 (skip=DSN 필요 postgres 통합) |
...
7. `.env*` 파일은 **읽기만 막혀 있다**(`Read(./.env.local)` deny). append 쓰기는 된다 — 기존 바이트를 건드리지 않는 추가만 하고, 값은 어떤 경우에도 출력하지 않는다(키 존재 여부만 확인). 2026-08-16 F-22에서 "쓸 수 없다"가 사실이 아님이 확인됐다.
...
**상태판은 [work_checklist.md](./work_checklist.md)의 「남은 외부 관문」이다.** 2026-08-17 기준 아직 열려 있는 것만 적는다.

| 관문 | 막히는 작업 | 필요한 것 |
|---|---|---|
| 주식장 개장 시간 + 키움 live 승인 | A-3 ③ | 장중 실행, CLAUDE.md 승인 항목 2 |
| 백업 암호문 VM 밖 보관 | F-25 후속 | 사용자가 비밀번호 관리자 등에 직접 보관 확인 |
| gold set 겹 C 보강 검수 | E-22 겹 C 승격 | 보강 910건의 `HUMAN_CONFIRMED` 라벨 확보(11.1.2절). 겹 A 1,150건은 검수 완료 |
| 2인 블라인드 평가 | E-19→E-20 | 사람 평가 통과 |
| 과거 주가 2005~2009 원천 | E-16 잔여 구간 | KRX Open API 미제공(실측) — 대체 원천 또는 범위 확정, 사용자 판단 |

해소된 관문: KRX·금감원(OpenDART) API 키(2026-08-15 A-2), 인포스탁·뉴스 live 호출(2026-08-16 D-14·B-8), `.env.example` 항목 추가(2026-08-16 B-8), 배포·cloud·DNS(2026-08-16 F-21·F-25).
...
- **이어서 할 일**: 당일 수집이 성공하면 **같은 키·같은 어댑터로 E-16(과거 전 종목 일봉) 백필을 백그라운드로 시작한다.** 1.5만 회 호출이라 며칠 걸리므로 출시 후에 시작하면 E-18이 밀린다. 범위와 필드는 아래 E-16 절 참조.
...
- 남은 것 없음 (2026-08-16 `5ae7a92`). ① live 수집 실행 ② 뉴스 Postgres 영속화 조립 ③ `.env.example`의 `NEWS_RSS_SOURCES`·뉴스 DSN·네이버/OpenAI 항목 — 셋 다 완료. 상세는 [work_checklist.md](./work_checklist.md) B-8 줄.
...
### E-16. 과거 주가 corpus — **2010-01-01~2026-08-14 구간 완료, 2026-08-16 `b4aac7d`·`76b0f2b`·`f6d5863`. 2005-03~2009-12는 외부 관문(KRX 미제공)**
...
### E-17. 사건·소재 온톨로지 (완료, 2026-08-16 `a30ba7e`)

> **아래 서술은 착수 전에 쓴 작업 정의다.** 실제 결과와 정확도 수치는 [work_checklist.md](./work_checklist.md) E-17 줄에 있다(통제어휘 v1.2.0, 소재 유형 28종, test split primary 엄격 77.8%·방향 99.6%·확실성 90.0%). 이 온톨로지를 회사 축으로 넓히는 후속 작업은 **E-22**다.

- 인포스탁 원인문의 표현들을 정규화된 소재 유형으로 묶는 통제어휘 + versioned transform. 위치: `research/ontology/**`, `packages/ontology/**`.
- **왜 하는가**: E-18(TOP3)과 E-19(유사사례)의 선행이면서, **E-21(자연어 리서치)의 핵심 선행**이다. 지금은 "테마 → 과거 사건"만 조회 가능하고 "소재 유형 → 어떤 테마가 반응했나"라는 역방향 질의가 불가능한데, 유형 라벨이 붙어야 그 축이 열린다.
- **재료는 이미 있다**: `data/infostock/import/**`의 테마 history 39,696건(2005-03 ~ 2026-08, 281테마). 원인문은 평균 55자 한 줄이고 20년간 서술 규격이 거의 같다(`[사유]에 상승/하락(주도주: ...)`). 날짜·테마·주도주·상승/하락 방향은 이미 구조화돼 있으므로 **분류 대상은 사유 한 조각뿐**이다. 전체 218만 자. 주가도 외부 키도 필요 없어 **A-2·E-16과 무관하게 지금 착수 가능하다.**
- **보조 재료**: `data/infostock/daily-full-20260814/`의 데일리 브리핑 4,655건(2007-09 ~ 2026-08, 7,476만 자, 결측 없음). 이미 `packages/infostock/daily.py`가 THEME_STOCK·DESCRIPTION 관계 144,961건으로 분해해 둔 상태다. 다만 전체 분량의 94%가 2022년 이후에 몰려 있어(2007년 평균 450자 → 2026년 89,333자) 과거 구간 맥락으로는 쓸 수 없고, 본문 파싱 부분 실패가 1,294건(28%) 있다.
- **설계 축**: 유형 하나만 붙이는 구조로는 부족하다. 최소 세 축이 필요하다 — ① 소재 유형 ② 상승/하락 방향(문장에 이미 있음) ③ **확정 사건 / 기대·전망** 구분. 원인문에 "기대감·전망·가능성·우려"가 반복적으로 나오는데, 확정 발표와 기대감은 이후 반응이 다르므로 섞으면 E-18 통계가 무의미해진다. 복합 사유(한 문장에 사유 2~3개)도 흔하므로 복수 라벨을 허용한다.
- **먼저 정할 것 — 유형 입자 크기**: 너무 잘면 유형당 사례가 통계 하한(30~50건) 아래로 떨어지고, 너무 굵으면 질의 해상도가 사라진다. 착수 시 ① 전체 39,696건 기계 빈도 분석(어절·키워드·연도별 변화)으로 유형 후보를 뽑고 ② 표본 1,000건을 읽어 이름을 붙이고 "기타" 비율을 잰다. **기타 10% 이하면 그 체계로 진행, 30% 이상이면 축을 다시 잡는다.** 표본만으로는 전체의 0.1%(약 40건) 규모 유형을 놓치므로 전수 빈도 분석을 먼저 한다.
- **한계로 기록해 둘 것**: 인포스탁이 기록한 사건만 다룬다. 기록 밀도가 거래일당 평균 9.1건인데 2010년 2.3건, 2024년 23.9건으로 편차가 크다. 과거 구간일수록 굵직한 사건만 남아 있다.
- LLM은 **유형 추출만** 한다. 점수·순위·반응 크기를 생성하지 않는다(`packages/llm/grounding.py`가 지키는 원칙과 동일).
...
- **목표**: `리서치` 탭(`/research`)에서 저장된 사건 기록을 자연어로 조회한다. 요구사항은 [PRD.md](./PRD.md) FR-11과 6.2 「자연어 리서치 질의」, 화면 규칙은 [screen_spec.md](./screen_spec.md) 11.7이다.
> **2026-08-17 정정 — 답할 질문의 정본은 E-22 계획서다.** [company_event_ontology_implementation_plan.md](./company_event_ontology_implementation_plan.md) 4.0절이 질의 유형 **17종**을 제품 결정으로 확정했고, 요청 방식도 **자연어 입력 하나**로 못박았다(날짜·종목을 클릭해 고르는 조회 화면은 만들지 않는다). 아래 「단계」의 1·2·3단계는 **데이터 확보 순서**를 뜻하는 것으로 남기고, 무엇을 구현할지는 계획서 17종과 E-22 단계 5를 따른다.

- **현재 상태**: 탭 자리와 시각 구성은 C-0에서 만들어졌다. 2026-08-15에 PRD·screen_spec에 요구사항을 작성했다(screen_spec 68행이 요구하던 선행 조건 해소).
  - **`DAY_MOVERS`(17종 중 1번)는 이미 나가 있다** — `ab02b80`이 `packages/infostock/daily_read.py`, `GET /v1/daily/movers`, `apps/web/src/pages/DayMoversPage.tsx`, 홈 진입 링크까지 만들었다. 계획서 결정 이전에 만든 조회 화면 형태라 **E-22 단계 5에서 조회 화면과 공개 엔드포인트를 제거하고** 읽기 함수만 QueryPlan 쪽으로 흡수한다(계획서 「단계 1에서 앞당겨 만든 것과 그 처리」).
  - 그 외 질의·응답 기능은 없다.
- **단계**: 데이터 확보 상태에 따라 답변 범위가 달라지므로 나눠서 낸다.
  1. **1단계 — 선행 없음.** 현재 시장·구조·근거 질문("지금 뭐가 오르나", "이 테마가 왜 오르나", "이 종목은 어느 테마인가"). FR-1~FR-4가 이미 계산하는 값을 질의 형태로 노출한다. **1차 출시(F-25)와 함께 낼 수 있다.**
  2. **2단계 — 선행 E-16 + E-17.** 과거 집계 질문("이 소재에 과거 어떤 테마가 반응했나", "몇 번이었고 그때 반응은 어땠나"). 여기서부터가 이 기능의 본체다.
  3. **3단계 — 선행 E-19 통과.** 유사사례 질문. 2인 블라인드 평가 전 잠금.
- **구조 원칙 (반드시 지킬 것)**:
  - 질문을 **닫힌 질의 유형 집합 + 슬롯 채우기**로 처리한다. 자유 형식 SQL 생성이나 문서 검색 기반 답변 생성을 쓰지 않는다.
  - 슬롯은 알려진 식별자로만 해석하고, 해석 실패 시 추측하지 않고 후보를 되묻는다(`grounding.py`의 `UNKNOWN_THEME`·`UNKNOWN_STOCK` 처리와 같은 규율).
  - **수치는 기존 계산 함수가 결정론적으로 산출하고 LLM은 문장 표현만 담당한다.** LLM이 수치·순위·확률을 만들지 않는다.
  - 답변마다 표본 수와 근거 사건 목록을 붙인다. 근거를 못 붙이면 답하지 않는다.
- **화면**: 질의 유형별로 화면을 따로 만들지 않는다. 블록 5종(해석 / 요약 문장 / 수치 요약 / 근거 사건 목록 / 미제시 사유)의 조합으로 전부 구성한다. 새 질의 유형이 생겨도 블록 재조합으로 처리한다. 상세는 screen_spec 11.7.
- **답할 수 없을 때**를 예외가 아니라 정식 응답으로 만든다. 사유를 **기록 없음 / 질문 해석 실패 / 제품 범위 밖** 셋으로 구분한다. 사건 기록 밀도가 거래일당 평균 9.1건(2010년 2.3건)이라 "기록 없음"이 자주 나온다.
- **범위 밖**: 미래 예측·매매 판단은 우회 표현으로도 제공하지 않는다(PRD 5.2). 질의 내용을 사용자 행동 분석 목적으로 축적하지 않는다.
- **완료 조건**: 1단계 질의 유형에 대해 슬롯 해석·계산·근거 표시가 동작하고, 해석 실패와 기록 없음이 구분되어 표시된다. `uv run pytest -q` 녹색, 웹 lint·typecheck 통과.

### E-22. 회사 중심 사건 온톨로지·자연어 질의 확장 ⭐ (진행 중, 2026-08-17 착수)

- **정본은 계획서다**: [company_event_ontology_implementation_plan.md](./company_event_ontology_implementation_plan.md). 이 절은 **목차와 상태만** 둔다. 무엇을 어떤 순서로 만드는지, 완료 조건이 무엇인지는 전부 계획서가 정한다. 이 절과 계획서가 어긋나면 계획서가 맞다.
- **무엇을 여는가**: 지금 온톨로지는 "이 소재에 어떤 테마가 반응했나"까지만 답한다. 회사가 사건의 주체인지 단순 주도주인지 구분하지 못해 "한화에어로스페이스가 직접 체결한 계약만" 같은 회사 질문에 답할 수 없다(실측: 그 회사 문자열이 나오는 테마 기록 156건 중 사유 본문 등장은 13건뿐). 이 작업이 회사·역할·사건·근거·실제 결과를 명시적으로 잇는다.
- **E-17·E-21과의 관계**: E-17(완료)의 4축 분류를 덮어쓰지 않고 회사 축으로 확장한다. E-21이 답할 질문 목록은 계획서 4.0절 **17종**이 정본이며, 요청 방식은 자연어 입력 하나다.
- **단계와 현재 상태** (계획서 9절, 2026-08-17 `f90e9c6` + 미커밋 워킹트리 기준):

| 단계 | 여는 것 | 상태 |
|---|---|---|
| 선행 | E-17 라벨을 DB에 적재해 SQL로 join | **완료** `9f5e2de` (`0007`) |
| 0 | 질의 17종 기계 계약 + gold set 3겹 | **부분** — 아래 잔여 |
| 1 | DailyFeaturedTheme 파싱 확장 | **부분** — 아래 잔여 |
| 2 | 회사 정체성·alias를 시점으로 분리 | **완료** `62a1b26`·`f90e9c6` (`0009`) |
| 3 | history 회사 역할 연결 | **진행 중** (`0010`·`company_roles.py` 미커밋) |
| 4 | 사건 구조·단계·중복 제거 | 미착수 |
| 5 | E-21 질의 17종 구현 | 미착수 |
| 6 | 금액·실제 결과 질문 | 미착수 (E-16 gate) |
| 7 | 유사사례 승격 | 미착수 (E-19 gate) |

- **단계 0 잔여**: ① 질의 ID·슬롯·집계 단위를 기계 enum으로 고정 — `packages/ontology/query_contracts.py`가 아직 없다. **enum은 정확히 17개다**(계획서 4.0절). 4.1.1절 후보 9종은 넣지 않는다. ② 겹 C 보강 910건은 사람 검수 전(`AI_DRAFT`·`AI_CROSS_CHECKED`)이라 계획서 11.1.2에 따라 해당 표본군으로는 승격 판정을 못 한다. 겹 A 1,150건은 2026-08-17 사용자 검수를 거쳐 `HUMAN_CONFIRMED`가 되었고, 겹 C 원본 1,000건은 E-17 블라인드 수동 라벨이며 `score_gold_set.py`가 5열 형식을 `HUMAN_CONFIRMED`로 읽어 채점한다.
...
- **외부 관문**: **겹 C 보강 gold set 사람 검수.** 겹 A는 검수 완료했으며, 겹 C 보강 표본군은 검수된 행이 없으면 계획서 11.2절 승격 기준을 어떤 수치로도 통과하지 못한다.
- **선행**: E-17(완료). 단계 6은 E-16, 단계 7은 E-19 gate에 각각 종속.
...
| E-22 회사 온톨로지·자연어 질의 | **Fable 5** | **max** |
...
```
A-3 ③(개장일 장중 관찰)        ← 외부 관문, 병렬
C-13(장중 상세 갱신)            ← 시연 계획 있으면 임시 해법 먼저

E-22 (진행 중)
  단계 0·1 잔여 → 단계 3 → 단계 4 → 단계 5(= E-21 질의 17종)
  → 단계 6(E-16 gate) → 단계 7(E-19 gate)
E-18(TOP3)  ← 선행 E-16(2010~ 구간)·E-17 모두 충족, 지금 착수 가능
E-19 → E-20 ← 2인 블라인드 통과 후에만
E-16 2005~2009 구간 ← 대체 원천 확보 여부는 사용자 판단
```

- **E-21은 더 이상 독립 항목으로 진행하지 않는다.** 질의 구현은 E-22 단계 5가 맡는다.
- **A-3 ③이 A-8 배선까지 실장중으로 확인한다.** `serve_live_api`가 거래일 세션 경로(`TradingDayLoop`)로 돌므로, 장중 소량 실행이 곧 그 경로의 검증이다. `scripts/verify_live_replay.py`로 개장 전에 녹화분 재생 확인도 가능하다.
- 1차 출시(A+B+C+D+F)는 2026-08-16에 끝났다 — https://dayjaview.vercel.app. 남은 것은 위 목록뿐이다.
- E는 출시 후 추가분이며, E-18(TOP3)이 E-19(유사사례)보다 먼저 나온다.
