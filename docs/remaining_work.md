...
- **현재 상태**: 탭 자리와 시각 구성은 C-0에서 만들어졌다. 질의·응답 기능은 없다. 2026-08-15에 PRD·screen_spec에 요구사항을 작성했다(screen_spec 68행이 요구하던 선행 조건 해소).
...
→ D-13 → D-14 → D-15
→ F-21 ~ F-25 (1차 출시)  [E-21 1단계를 여기 붙일 수 있음]
→ E-16 → E-17 → E-18(TOP3) → E-19 → E-20 (출시 후 업데이트)
→ E-21 2단계(E-16+E-17 후) → E-21 3단계(E-19 통과 후)

E-17은 외부 관문이 없다 (주가·KRX 키 불필요, 재료가 이미 디스크에 있음).
→ A-3 장중 대기나 F 출시 준비와 병렬로 착수 가능.
```
