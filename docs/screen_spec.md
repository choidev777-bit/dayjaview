...
| 리서치 | — | `/research` | 과거 데이터 자연어 질의 | 자리만 — 요구사항 미작성 |

- `즐겨찾기` 탭은 핵심 구현 대상이며 비활성 placeholder로 데모하지 않는다.
- `리서치` 탭은 시안에만 있고 [PRD.md](./PRD.md)에 없다. 화면 자리와 시각 구성만 구현하고 질의·응답 기능은 만들지 않는다. 착수 전에 PRD와 이 문서에 요구사항을 먼저 쓴다.
...
| 리서치 | 과거 데이터 자연어 질의 | 자리만 — 요구사항 미작성 |
