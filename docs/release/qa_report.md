...
- **수리(2026-08-16, 점검 이후)**: 발견 1·2를 고쳤다. 3·4·5는 그대로다. 아래 각 절의 본문은 *수리 전 상태* 설명이고, 무엇을 고쳤는지는 절 끝의 **수리** 항목이 기준이다.
...
| 2 | 중간 | `/auth/*` 요청 한도가 인증된 `/auth/session`까지 함께 세고 프록시 뒤에서 전 사용자가 한 통을 공유한다 | 이번 E2E 중 브라우저 하나로 429 발생 | **수리** |
...
**수리 (2026-08-16)**: 두 갈래를 나눠 고쳤다.

- **세는 대상**: 한도를 `/auth/` 접두사 전체가 아니라 무인증으로 저장소를 건드리는 OAuth 진입점 둘(`/auth/google`, `/auth/google/callback`)에만 건다(`apps/api/app.py:76-79`). 쿠키가 있어야 하고 행을 만들지 않는 `/auth/session`·`/auth/logout`은 예산을 먹지 않는다. F-23이 이 한도를 넣은 이유(무인증 진입점이 요청마다 `oauth_states` 행을 만드는 것)는 그대로 지킨다.
- **세는 단위**: 앞단 프록시 수를 `TRUSTED_PROXY_HOPS`로 **선언한 경우에만** `X-Forwarded-For`의 그 자리 값을 쓴다(`apps/api/http.py:67-86`). 선언이 없으면(기본 0) 지금처럼 전송 계층 주소만 쓰고 header는 쳐다보지 않는다 — 위조 가능한 값을 기본으로 믿지 않는다. 항목 수가 선언한 hop 수보다 적으면 전송 계층 주소로 되돌아간다(fail closed).
- 회귀 테스트 2건(`tests/security/test_auth_endpoint_abuse.py`): 세션 조회를 한도의 3배로 두들겨도 로그인 시작 예산이 남아 있는 것, 프록시 hop을 선언하면 뒤쪽 사용자별로 따로 세고 선언하지 않으면 위조 header를 무시하는 것.
- 실행 중인 서버로 재확인: `/auth/session` 60회 연속 전부 200이고 그 뒤 로그인 시작이 302다. 로그인 시작 자체를 연속 25회 하면 19회 뒤 429로 끊긴다(남용 차단은 유지).
- **F-25에서 정해야 할 것**: Vercel `/api/*` rewrite로 API를 감싸면 hop이 1 이상이 된다. 배포 구성을 확정할 때 `TRUSTED_PROXY_HOPS`를 실제 앞단 수로 넣어야 사용자별로 세어진다. 값을 넣지 않으면 동작은 지금과 같고(전 사용자 한 통) 로그인 시작에만 걸린다.
