...
- **원칙**: 제품 파일은 고치지 않았다. 실행한 검증과 재현 조건, 발견만 남긴다. **수리는 별도 작업이다.**
...
| # | 심각도 | 항목 | 재현 |
|---|---|---|---|
| 1 | **높음(배포 차단)** | Coverage 100% 테마에서 `observed_weight_ratio`가 1을 미세하게 넘겨 publish 전체가 ValueError로 죽고, 발행 루프가 조용히 멈춘다 | 실규모 우주 5주기 내 |
| 2 | 중간 | `/auth/*` 요청 한도가 인증된 `/auth/session`까지 함께 세고 프록시 뒤에서 전 사용자가 한 통을 공유한다 | 이번 E2E 중 브라우저 하나로 429 발생 |
| 3 | 중간 | 색상 대비 WCAG AA 미달 23곳 (6화면 중 5화면) | axe-core 실행 중 화면 |
| 4 | 정보성 | 웹 접근성 테스트가 `color-contrast` 규칙을 끈 채 통과한다 — 3번은 CI에서 절대 안 걸린다 | `apps/web/src/test/accessibility.test.tsx:16` |
| 5 | 정보성 | publish 한 주기가 실규모에서 0.33~1.0초. 85%가 `_select_references` 선형 스캔 | cProfile |
...
- 전 종목 확산 수치는 reference가 비었던 조건에서 완주한 값이다. reference를 채운 같은 프로파일은 **발견 1로 중단**돼 완주하지 못했다.
