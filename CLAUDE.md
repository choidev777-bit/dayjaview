...
- **예외 — 체크리스트는 갱신한다.** `docs/remaining_work.md`의 작업 번호(A-1, B-9 …)를 하나 끝내면 [work_checklist.md](docs/work_checklist.md)에서 그 줄의 `[ ]`를 `[x]`로 바꾸고 `— 완료일 commit해시`를 적는다. 문서 맨 위 "마지막 갱신"과 요약 표의 완료 수도 같이 고치고, 같은 commit에 포함한다. 다 못 끝냈으면 `[ ]`로 두고 남은 것을 한 줄로 적는다. 여기 말고 다른 문서에는 실행 기록을 쓰지 않는다.
...
| 목적 | 명령 |
|---|---|
| 파이썬 테스트 | `uv run pytest -q` (2026-08-17 기준 642 passed·10 skipped, E-22 단계 3 미커밋분 포함; skip은 DSN 필요 postgres 통합 테스트) |
