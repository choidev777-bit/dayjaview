...
from .research import MAX_QUESTION_LENGTH, ResearchService
...
class IdentityApiApp:
    ...
    def _handle(self, request: ApiRequest, request_id: str) -> ApiResponse:
        ...
        if request.path == "/v1/research/answers" and request.method == "POST":
            return self._research_answer(request, request_id)
    ...
    def _research_answer(self, request: ApiRequest, request_id: str) -> ApiResponse:
        """자연어 질문 하나를 받아 답변 블록 하나로 답한다.

        질문 문장을 URL에 싣지 않으려고 POST로 받는다. 저장은 하지 않으며
        다른 POST와 같은 CSRF 검사를 받는다.
        """

        self.identity_service.authorize_question(
            session_token=request.cookies.get(SESSION_COOKIE),
            origin=request.header("origin"),
            csrf_token=request.header("x-csrf-token"),
            csrf_cookie=request.cookies.get(CSRF_COOKIE),
        ...
        if self._research_service is None:
            raise ProductDataUnavailable
        question = self._question_text(request)
        answer = self._research_service.answer(
            question, today=self._clock.now().astimezone(KST).date()
        )
        return self._success(200, answer, request_id)

    @staticmethod
    def _question_text(request: ApiRequest) -> str:
        try:
            payload = json.loads(request.body or "{}")
        except ValueError as error:
            raise InvalidApiRequest("질문 형식을 확인해 주세요.") from error
        if not isinstance(payload, dict) or set(payload) - {"question"}:
            raise InvalidApiRequest("질문 형식을 확인해 주세요.")
        question = payload.get("question")
        if not isinstance(question, str) or not question.strip():
            raise InvalidApiRequest("질문을 입력해 주세요.")
        if len(question) > MAX_QUESTION_LENGTH:
            raise InvalidApiRequest("질문이 너무 깁니다.")
        return question
